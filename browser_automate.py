...
class Win_BrowserAutomate :
    # 此配置表格中的内容项是固定的 所以写死在代码里
    CFG_ITEMS = [
        '选择',"地址","私钥","助记词",'用户信息目录',"操作"
    ]

    def __init__(self):
        self.ui = QUiLoader().load('browser_automate.ui')

        # 读取Excel文件
        self.ui.btn_read_excel.clicked.connect(self.onReadExcel)

        # 定義信號處理函數
        mysingnal_log.log.connect(self.onLog)
